...
if (str.length==0) { 
    ...
    xmlhttp.open("GET","ajax_hint.php?q="+str,true);
    xmlhttp.send();
}    
}
</script>
...
<form action="log_add.php?time=<?php if (isset($_GET['time'])){ echo $_GET['time']; };?>" method="post">
  name of food: (<span id="txtHint">do you mean??</span> )<br>
  <input type="text" name="name" onkeyup="showHint(this.value)" value="<?php if(isset($name)){echo $name;}; ?>">(example: banana) <br><br>
