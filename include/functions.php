...
function query_logread($current_day_ts, $userid){
	$timemin = floor($current_day_ts / (3600 * 24)) * 3600 * 24;
	$timemax = $timemin + 86399;
...
function gettime(){
	global $posttime_ajax;
	 if (isset($_GET['time'])) {
      $utctimestamp = htmlspecialchars($_GET['time']);
      $datemath = fdate($utctimestamp);
      $formatteddate = date("D, d M Y", $utctimestamp);
    }elseif (isset($_POST['logtime'])) {
      $utctimestamp = htmlspecialchars($_POST['logtime']);
      $datemath = fdate($utctimestamp);
      $formatteddate = date("D, d M Y", $utctimestamp);
    }elseif (isset($posttime_ajax)) {
      $utctimestamp = htmlspecialchars($posttime_ajax);
      $datemath = fdate($utctimestamp);
      $formatteddate = date("D, d M Y", $utctimestamp);
    } else{
    $utctimestamp = time();
